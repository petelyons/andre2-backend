      ...
      if (Array.isArray(arr)) {
        ...
        for (const t of arr) {
          ...
          if (t.sessionId && !t.userEmail) {
            ...
            if (session) {
              const email =
                session.state?.spotify?.email || session.state?.listener?.email;
              if (email) {
                t.userEmail = email;
                delete t.sessionId;
              } else {
...
function broadcastTrackList() {
  ...
  for (const [sessionId, session] of sessions.entries()) {
    if (!session.ws || session.ws.readyState !== 1) {
      // 1 = OPEN
      logger.info(
        `Skipping broadcast to session ${sessionId} (WebSocket not open)`,
      );
    ...
    session.ws &&
      session.ws.send(
        JSON.stringify({ type: "tracks_list", tracks: trackList }),
      );
  }
}
  ...
  if (session && session.ws && session.ws.readyState === 1) {
    session.ws &&
      session.ws.send(
        JSON.stringify({
          type: "session_mode",
          sessionMode: sessionModes.get(sessionId) || "session_pause",
        }),
      );
  }
}
...
function broadcastSessionList() {
  ...
  for (const [sessionId, session] of sessions.entries()) {
    ...
    if (
      !emailToSession.has(email) ||
      sessionId > emailToSession.get(email)!.sessionId
    ) {
  ...
  for (const [sessionId, session] of sessions.entries()) {
    ...
    session.ws &&
      session.ws.send(
        JSON.stringify({ type: "sessions_list", sessions: sessionList }),
      );
  }
}
  ...
  if (!masterUserSessionId) {
    for (const [sessionId, session] of sessions.entries()) {
      if (session.state?.spotify?.access_token) {
        ...
        try {
          const playback = await spotifyDelegate.getMyCurrentPlaybackState(
            session.state.spotify.access_token,
          );
          ...
          if (item && item.type === "track") {
            ...
            currentlyPlayingTrack = {
              ...
              jammers: [],
              progress:
                playback.body.progress_ms && item.duration_ms
                  ? {
            ...
            logger.info(
              `Set currentlyPlayingTrack from master user's current playback: ${trackInfo.name} by ${trackInfo.artist}`,
            );
            ...
            if (typeof playback.body.is_playing === "boolean") {
              ...
              logger.info(
                `Set play mode to ${mode} based on detected master playback state`,
              );
              broadcastMode();
              if (mode === "master_play") {
                startPolling();
          ...
          logger.warn(
            "Could not fetch master user playback state on assignment:",
            err,
          );
        }
        break;
  ...
  if (session && session.ws && session.ws.readyState === 1) {
    session.ws.send(
      JSON.stringify({
        ...
        message:
          "Spotify needs to be activated. Please hit play on your Spotify client to activate your device.",
      }),
    );
  }
}
...
async function pollMasterUserPlayback() {
  ...
  try {
    const playback = await spotifyDelegate.getMyCurrentPlaybackState(
      masterSession.state.spotify.access_token,
    );
    if (isDebug()) {
      logger.info(
    ...
    const curr = {
      ...
      progress_ms:
        typeof playback.body?.progress_ms === "number"
          ? playback.body.progress_ms
          : null,
      duration_ms:
        typeof playback.body?.item?.duration_ms === "number"
          ? playback.body.item.duration_ms
          : null,
      is_playing:
        typeof playback.body?.is_playing === "boolean"
    ...
    if (
      ...
      curr.uri &&
      prev.uri === curr.uri &&
      prev.progress_ms !== null &&
      curr.progress_ms === 0 &&
      prev.duration_ms &&
      prev.progress_ms > 0.9 * prev.duration_ms
    ) {
    ...
    if (detectedTrackEnd) {
      ...
      if (submittedTracks.length > 0) {
        ...
        saveTracksToFile();
        logger.info(
          `Now playing: ${currentlyPlayingTrack.name || currentlyPlayingTrack.spotifyUri}`,
        );
        ...
        for (const [sid, sess] of sessions.entries()) {
          ...
          if (sessionMode === "session_play") {
            ...
            if (accessToken) {
              try {
                await spotifyDelegate.play(accessToken, {
                  uris: [currentlyPlayingTrack.spotifyUri],
                });
                logger.info(
                  `Auto-started playback for session ${sid} (session_play mode)`,
                );
              } catch (err) {
                logger.error(
                  `Failed to auto-start playback for session ${sid}:`,
                  err,
                );
                const errMsg = getErrorMessage(err);
                if (
                  errMsg.includes("NO_ACTIVE_DEVICE") ||
                  errMsg.includes("No active device found")
                ) {
      ...
      if (currentlyPlayingTrack) {
        playHistory.push({
          ...
          startedBy: masterUserSessionId
            ? sessions.get(masterUserSessionId)?.state?.spotify?.name ||
              sessions.get(masterUserSessionId)?.state?.listener?.name ||
              ""
            : undefined,
        });
    ...
    if (mode === ("master_pause" as typeof mode)) {
      ...
      if (
        currentlyPlayingTrack &&
        curr.progress_ms !== null &&
        curr.duration_ms !== null
      ) {
    ...
    if (curr.is_playing !== null && curr.is_playing !== undefined) {
      ...
      if (currentlyPlayingTrack && curr.uri) {
        ...
        if (masterTrackUri !== currentlyPlayingTrack.spotifyUri) {
          ...
          logger.warn(
            `Master user is playing ${masterTrackUri} but should be playing ${currentlyPlayingTrack.spotifyUri}. Attempting to correct...`,
          );
          try {
            await spotifyDelegate.play(
              masterSession.state.spotify.access_token,
              { uris: [currentlyPlayingTrack.spotifyUri] },
            );
            logger.info(
              `Successfully started correct track (${currentlyPlayingTrack.spotifyUri}) for master user.`,
            );
          } catch (err) {
      ...
      if (
        ...
        const progressPercentage =
          currentlyPlayingTrack.progress.position_ms /
          currentlyPlayingTrack.progress.duration_ms;
        if (isDebug()) {
          logger.info(
        ...
        if (progressPercentage >= 1.0) {
          logger.info(
            `Track "${currentlyPlayingTrack.name}" has finished playing, moving to next track`,
          );
      ...
      if (newState !== lastMasterPlaybackState) {
        ...
        if (newState === "play" && mode !== "master_play") {
          ...
          if (currentlyPlayingTrack) {
            ...
            logger.info(
              `Master user started playing track: ${currentlyPlayingTrack.name}`,
            );
          }
          broadcastMode();
          ...
          if (
            ...
            if (progressPercentage >= 1.0) {
              ...
              logger.info(
                `Track "${currentlyPlayingTrack.name}" completed and paused, advancing to next track (not setting master_pause).`,
              );
...
function startPolling() {
  if (!pollInterval)
    pollInterval = setInterval(pollMasterUserPlayback, POLL_INTERVAL_MS);
}
...
function startWebSocketServer(retries = 3, delay = 2000): void {
  try {
        ...
        try {
          ...
          switch (message.type) {
            case "login":
              ...
              const listenerEmail = session.state?.listener?.email;
              if (
                !(spotifyName && spotifyEmail) &&
                !(listenerName && listenerEmail)
              ) {
            ...
            case "remove_track":
              ...
              if (message.spotifyUri && message.sessionId) {
                const trackIndex = submittedTracks.findIndex(
                  (t) => t.spotifyUri === message.spotifyUri,
                );
                if (trackIndex !== -1) {
                  ...
                  const removerName =
                    removerSession?.state?.spotify?.name || "Unknown User";
                  const removerEmail =
                    removerSession?.state?.spotify?.email || "No email";
                  ...
                  logger.info(
                    `Track removed: "${trackName}" by ${trackArtist} (${trackAlbum}) [${message.spotifyUri}] - Removed by ${removerName} (${removerEmail}) [${message.sessionId}]`,
                  );
                  broadcastTrackList();
                }
              }
            ...
            case "jam":
              if (message.spotifyUri && message.sessionId) {
                ...
                const jammerEmail =
                  jammerSession?.state?.spotify?.email ||
                  jammerSession?.state?.listener?.email ||
                  "";
                if (!jammerEmail) return; // Don't allow jam if no email
                // Update in queue
                const track = submittedTracks.find(
                  (t) => t.spotifyUri === message.spotifyUri,
                );
                let jammed = false;
                if (track) {
                ...
                if (
                  ...
                  if (!Array.isArray(currentlyPlayingTrack.jammers))
                    currentlyPlayingTrack.jammers = [];
                  const idx =
                    currentlyPlayingTrack.jammers.indexOf(jammerEmail);
                  if (idx === -1) {
                ...
                if (updated) {
                  logger.info(
                    `User ${jammerEmail} toggled jam for track ${message.spotifyUri}`,
                  );
                  const jammerName =
                    jammerSession?.state?.spotify?.name ||
                    jammerSession?.state?.listener?.name ||
                    "Unknown";
                  history.push({
                    ...
                    if (accessToken) {
                      try {
                        ...
                        if (
                          ...
                          playOptions.position_ms =
                            currentlyPlayingTrack.progress.position_ms;
                        }
                        await spotifyDelegate.play(accessToken, playOptions);
                        logger.info(
                          `Started playback for session ${sid} from ${playOptions.position_ms ? `position ${playOptions.position_ms}` : "beginning"}`,
                        );
                      } catch (err) {
                        logger.error(
                          `Failed to start playback for session ${sid}:`,
                          err,
                        );
                        const errMsg = getErrorMessage(err);
                        if (
                ...
                for (const [sid, sess] of sessions.entries()) {
                  ...
                  if (accessToken) {
                    try {
                      ...
                      logger.error(
                        `Failed to pause Spotify for session ${sid}:`,
                        err,
                      );
                    }
                  }
                ...
                if (currentlyPlayingTrack && currentlyPlayingTrack.spotifyUri) {
                  ...
                  if (accessToken) {
                    try {
                      ...
                      logger.info(
                        `Session ${message.sessionId} started playback from beginning`,
                      );
                    } catch (err) {
                      logger.error(
                        `Failed to start playback for session ${message.sessionId}:`,
                        err,
                      );
                    }
                  }
            ...
            case "session_pause":
              if (message.sessionId) {
                ...
                logger.info(
                  `Session ${message.sessionId} set to session_pause`,
                );
                ...
                if (accessToken) {
                  try {
                    await spotifyDelegate.pause(accessToken);
                    logger.info(
                      `Paused Spotify playback for session ${message.sessionId}`,
                    );
                  } catch (err) {
                    logger.error(
                      `Failed to pause Spotify for session ${message.sessionId}:`,
                      err,
                    );
                  }
                }
            ...
            case "airhorn":
              if (message.airhorn) {
                ...
                const senderName =
                  senderSession?.state?.spotify?.name ||
                  senderSession?.state?.listener?.name ||
                  "Unknown";
                const senderEmail =
                  senderSession?.state?.spotify?.email ||
                  senderSession?.state?.listener?.email ||
                  "";
                history.push({
                  type: "airhorn",
                  timestamp: Date.now(),
            ...
            case "master_skip":
              if (
                message.sessionId &&
                message.sessionId === masterUserSessionId
              ) {
                ...
                if (currentlyPlayingTrack) {
                  playHistory.push({
                    ...
                    track: { ...currentlyPlayingTrack },
                    startedBy: masterUserSessionId
                      ? sessions.get(masterUserSessionId)?.state?.spotify
                          ?.name ||
                        sessions.get(masterUserSessionId)?.state?.listener
                          ?.name ||
                        ""
                      : undefined,
                  });
    ...
    if (retries > 0) {
      logger.info(
        `Failed to start WebSocket server. Retrying in ${delay / 1000} seconds... (${retries} attempts remaining)`,
      );
      setTimeout(() => startWebSocketServer(retries - 1, delay), delay);
    } else {
      logger.error(
        "Failed to start WebSocket server after multiple attempts:",
        error,
      );
      process.exit(1);
    }
...
app.post("/api/tracks", async (req: Request, res: Response): Promise<void> => {
  ...
  const accessToken = session?.state?.spotify?.access_token;
  let trackInfo: any = {};
  const masterSession = masterUserSessionId
    ? sessions.get(masterUserSessionId)
    : null;
  ...
  if (masterAccessToken && trackId) {
    try {
      ...
      if (trackInfo && trackInfo.name && trackInfo.artist && trackInfo.album) {
        logger.info(
          `Track info: ${trackInfo.name} | ${trackInfo.artist} | ${trackInfo.album}`,
        );
      }
    } catch (err) {
  ...
  logger.info(
    `Track submitted: ${spotifyUri} by session ${sessionId} (${submitterName})`,
  );
  fairInsertTrack(
    {
      spotifyUri,
      userEmail: submitterEmail,
      spotifyName: submitterName,
      timestamp: Date.now(),
      ...trackInfo,
    },
    submittedTracks,
  ...
  if (
    ...
    ((session.state.spotify && session.state.spotify.access_token) ||
      (session.state.listener &&
        session.state.listener.name &&
        session.state.listener.email))
  ) {
...
async function refreshAllSpotifyTokens() {
  for (const [sessionId, session] of sessions.entries()) {
    ...
    if (spotifyState && spotifyState.refresh_token) {
      try {
        const data = await spotifyDelegate.refreshAccessToken(
        ...
        if (data && data.access_token) {
          ...
          if (data.refresh_token)
            spotifyState.refresh_token = data.refresh_token;
          logger.info(
            `Refreshed Spotify access token for session ${sessionId}`,
          );
        }
      } catch (err) {
        logger.error(
          `Failed to refresh Spotify token for session ${sessionId}:`,
          err,
        );
      }
    }
...
async function masterRandomLikedHandler(req: Request, res: Response) {
  ...
  if (!sessionId || sessionId !== masterUserSessionId) {
    return res
      .status(403)
      .json({ error: "Only the master can use this feature." });
  }
  const session = sessions.get(sessionId);
  const accessToken = session?.state?.spotify?.access_token;
  ...
  if (!accessToken) {
    return res
      .status(400)
      .json({ error: "Master does not have a valid Spotify session." });
  }
  try {
    const tracks = await spotifyDelegate.getRandomLikedTracks(accessToken, 10);
    for (const track of tracks) {
      // Prevent duplicates
      if (submittedTracks.some((t) => t.spotifyUri === track.spotifyUri))
        continue;
...
function broadcastPlayHistory() {
  ...
  for (const [sessionId, session] of sessions.entries()) {
    if (!session.ws || session.ws.readyState !== 1) continue;
    session.ws.send(
      JSON.stringify({ type: "play_history", playHistory: historyList }),
    );
  }
}
          ...
          if (state && state.spotify && state.spotify.refresh_token) {
            try {
              ...
              if (data && data.access_token) {
                ...
                if (data.refresh_token)
                  state.spotify.refresh_token = data.refresh_token;
                logger.info(
                  `Refreshed Spotify access token for session ${sessionId}`,
                );
              }
